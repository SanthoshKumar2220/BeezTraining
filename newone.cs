...
namespace Shiphol
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            String filename = @"C:\Schiphol\MyTest.csv";
            ...
            for (int i = 0; i < product.Count; i++)
            {
                ...
                String productName = product1[i].Text;
                String[] strr = productName.Split();
                ...
                try
                {
                    ...
                    String edit = prices1[i].Text.Replace("Price from:", "");
                    String Current=edit.Replace("Current price:", "");
                    String price= Current.Replace("Price:", "");
                    String Discount=price.Replace("Discount:", "");
                    ...
                    String tests = ",";
                    using (StreamWriter swriter = System.IO.File.CreateText(filename))
                    {
                        swriter.WriteLine($"Page URL{tests}productName{tests}" +
                            $"Quantity{tests}price{tests}Current{tests}" +
                            $"edit{tests}Discounted Price{tests}Discount {tests}");
                        foreach(String words in strr)
                        {
                            swriter.WriteLine($"{productName}tests" +
                           $"tests{quantity}tests{price}test{Current}" +
                           $"tests{edit}tests{Discount}");
                        }
